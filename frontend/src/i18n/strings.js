const STRINGS = {
  sv: {
    appName: "Ordning",
    sidebarTitle: "Kalendrar",
    sidebarPlaceholder: "Inga kalendrar ännu",
    newEventButton: "Ny händelse",
    exportButton: "Exportera",
    importButton: "Importera",
    todayButton: "Idag",
    saveButton: "Spara",
    saveButtonSaving: "Sparar...",
    saveButtonSaved: "Sparat ✓",
    printButton: "Skriv ut",
    toolbarPreviousWeek: "Föregående vecka",
    toolbarNextWeek: "Nästa vecka",
    searchPlaceholder: "Sök händelser",
    searchNoResults: "Inga träffar",
    miniMonthPrevious: "Föregående månad",
    miniMonthNext: "Nästa månad",
    calendarGroupUngrouped: "Mina kalendrar",
    calendarCreatePlaceholder: "Nytt kalendernamn",
    calendarGroupPlaceholder: "Grupp (valfritt)",
    calendarCreateButton: "Lägg till kalender",
    calendarCreateDialogTitle: "Ny kalender",
    calendarCreateOpenButton: "Skapa kalender",
    calendarEditButton: "Redigera",
    calendarEditDialogTitle: "Redigera kalender",
    calendarDeleteButton: "Ta bort",
    calendarDeleteConfirm: "Ta bort kalendern och alla dess händelser?",
    calendarDeleteError: "Kalendern kunde inte tas bort.",
    calendarColorLabel: "Kalenderfärg",
    calendarGroupLabel: "Kalendergrupp",
    calendarVisibilityError: "Minst en kalender måste vara synlig.",
    weekGridNoCalendarsTitle: "Inga kalendrar",
    weekGridNoCalendarsHint: "Skapa en kalender i sidopanelen för att börja planera.",
    weekOfPrefix: "Vecka",
    daysShort: ["Sön", "Mån", "Tis", "Ons", "Tor", "Fre", "Lör"],
    eventFormCreateHeading: "Ny händelse",
    eventFormEditHeading: "Redigera händelse",
    eventFormTemplateNoResults: "Inga mallar hittades",
    eventFormTemplateAllDay: "Heldag",
    eventFormTemplateDays: "{count} dagar",
    eventFormTitle: "Titel",
    eventFormCalendar: "Kalender",
    eventFormCalendarRequired: "Välj en kalender.",
    eventFormNoCalendars: "Skapa en kalender först.",
    eventFormNoCalendarsPrompt: "Du behöver minst en kalender innan du kan skapa en händelse.",
    eventFormFocusCalendarCreate: "Gå till kalender-skapande",
    eventFormStartDate: "Startdatum",
    eventFormEndDate: "Slutdatum",
    eventFormStartTime: "Starttid",
    eventFormEndTime: "Sluttid",
    eventFormAllDay: "Heldag",
    eventFormLocation: "Plats",
    recurrenceRepeat: "Upprepa",
    recurrenceNone: "Upprepas inte",
    recurrenceWeekly: "Varje vecka",
    recurrenceMonthly: "Varje månad",
    recurrenceEvery: "Varje",
    recurrenceWeeks: "vecka/veckor",
    recurrenceMonths: "månad/månader",
    recurrenceOn: "På",
    recurrenceOnThe: "Den",
    recurrenceEnds: "Slutar",
    recurrenceNever: "Aldrig",
    recurrenceAfter: "Efter",
    recurrenceOccurrences: "gånger",
    recurrenceOnDate: "Vid datum",
    recurrenceOrdinal1: "1:a",
    recurrenceOrdinal2: "2:a",
    recurrenceOrdinal3: "3:e",
    recurrenceOrdinal4: "4:e",
    recurrenceOrdinal5: "5:e",
    recurrenceEditPrompt: "Tillämpa ändringen på alla framtida händelser eller bara den här?",
    recurrenceEditAllFuture: "Alla framtida",
    recurrenceEditJustThisOne: "Bara den här",
    bulkDescriptionPrompt: "{count} andra händelser har samma titel. Uppdatera beskrivningen på alla?",
    bulkDescriptionUpdateAll: "Uppdatera alla",
    bulkDescriptionOnlyThis: "Bara den här",
    eventFormPrivateDescription: "Privat beskrivning",
    eventFormPrivateDescriptionTooltip: "Privat — exkluderas från publika exporter",
    eventFormPublicDescription: "Publik beskrivning",
    eventFormDelete: "Ta bort",
    eventFormCancel: "Avbryt",
    eventFormSave: "Spara",
    eventFormDeleteConfirm: "Ta bort den här händelsen?",
    exportDialogTitle: "Exportera kalender",
    exportModeLabel: "Exportläge",
    exportModeFull: "Full export",
    exportModePublic: "Offentlig export",
    exportModeFullSubtitle: "Både privata och offentliga händelser",
    exportModePublicSubtitle: "Endast offentliga händelser",
    exportEncryptLabel: "Kryptera export",
    exportEncryptHint: "Skydda filen med lösenord",
    exportPasswordLabel: "Lösenord",
    exportPasswordConfirmLabel: "Bekräfta lösenord",
    exportCalendarsLabel: "Kalendrar",
    exportPreviewCount: "{count} händelser kommer exporteras.",
    exportPreviewNone: "Välj minst en kalender.",
    exportCalendarRequired: "Välj minst en kalender.",
    exportPasswordRequired: "Fyll i lösenord och bekräftelse.",
    exportPasswordMismatch: "Lösenorden matchar inte.",
    exportCancel: "Avbryt",
    exportConfirm: "Exportera",
    exportSuccessMessage: "Export klar: {path}",
    unlockTitle: "Ordning",
    unlockMessage: "Din data är krypterad. Ange lösenord för att låsa upp.",
    unlockPasswordLabel: "Lösenord",
    unlockButton: "Lås upp",
    unlockWrongPassword: "Fel lösenord.",
    unlockQuit: "Avsluta",
    importDialogTitle: "Importera kalender",
    importStrategyLabel: "Importstrategi",
    importStrategyMerge: "Slå ihop",
    importStrategyReplace: "Ersätt allt",
    importPickFile: "Välj fil",
    importNoFileSelected: "Ingen fil vald.",
    importPasswordLabel: "Lösenord (för krypterad fil)",
    importEncryptedFile: "Krypterad fil",
    importUnlockButton: "Lås upp",
    importUnlocked: "Upplåst",
    importPasswordHint: "Lämna tomt för okrypterad JSON.",
    importSummaryCalendars: "Kalendrar i fil: {count}",
    importSummaryEvents: "Händelser i fil: {count}",
    importSummaryNew: "Nya händelser: {count}",
    importSummaryUpdated: "Uppdaterade händelser: {count}",
    importSummaryConflicts: "Konflikter: {count}",
    importCancel: "Avbryt",
    importConfirm: "Importera",
    importSuccessMessage:
      "Import klar. Nya: {new}, uppdaterade: {updated}, konflikter: {conflicts}",
    offscreenMore: "Fler händelser",
    contextMenuOpen: "Öppna",
    contextMenuDelete: "Ta bort",
    contextMenuCopy: "Kopiera",
    contextMenuNewEvent: "Ny händelse",
    contextMenuPaste: "Klistra in",
    contextMenuCopyError: "Kunde inte kopiera händelsen.",
    purgePastButton: "Rensa gamla händelser",
    purgePastPreview: "Raderar alla händelser i kalendern innan dagens datum.",
    purgePastFinalConfirm: "Detta kan inte ångras. Fortsätt?",
    purgePastFinalCancel: "Gamla händelser faller (inte) av",
    purgePastFinalProceed: "Framhärda",
    purgePastSuccess: "{count} gamla händelser togs bort.",
    purgePastNone: "Inga gamla händelser att rensa.",
    purgePastError: "Kunde inte rensa gamla händelser.",
    printWeekButton: "Skriv ut vecka",
    settingsButtonAria: "Öppna inställningar",
    settingsTitle: "Inställningar",
    settingsLanguageLabel: "Språk",
    settingsTimezoneLabel: "Tidszon",
    settingsLanguageSwedish: "Svenska",
    settingsLanguageEnglish: "English",
    settingsEncryptionTitle: "Kryptering",
    settingsEncryptionOff: "Lokal data är inte krypterad.",
    settingsEncryptionOn: "Lokal data är krypterad.",
    settingsEncryptButton: "Kryptera lokal data",
    settingsEncryptConfirmLabel: "Bekräfta lösenord",
    settingsEncryptSubmit: "Kryptera",
    settingsEncryptCancel: "Avbryt",
    settingsEncryptPasswordMismatch: "Lösenorden matchar inte.",
    settingsEncryptPasswordRequired: "Lösenord krävs.",
    settingsEncryptedStatus: "On",
    settingsDisableEncrypt: "Avaktivera kryptering",
    multiDeleteConfirm: "Ta bort {count} händelser?",
    multiDeleteRecurringNote: "Återkommande instanser tas bort enskilt.",
    contextMenuDeleteCount: "Ta bort ({count})",
    closeUnsavedPrompt: "Du har osparade ändringar. Vad vill du göra?",
    closeUnsavedSave: "Spara",
    closeUnsavedDontSave: "Spara inte"
  },
  en: {
    appName: "Ordning",
    sidebarTitle: "Calendars",
    sidebarPlaceholder: "No calendars yet",
    newEventButton: "New event",
    exportButton: "Export",
    importButton: "Import",
    todayButton: "Today",
    saveButton: "Save",
    saveButtonSaving: "Saving...",
    saveButtonSaved: "Saved ✓",
    printButton: "Print",
    toolbarPreviousWeek: "Previous week",
    toolbarNextWeek: "Next week",
    searchPlaceholder: "Search events",
    searchNoResults: "No results",
    miniMonthPrevious: "Previous month",
    miniMonthNext: "Next month",
    calendarGroupUngrouped: "My calendars",
    calendarCreatePlaceholder: "New calendar name",
    calendarGroupPlaceholder: "Group (optional)",
    calendarCreateButton: "Add calendar",
    calendarCreateDialogTitle: "New calendar",
    calendarCreateOpenButton: "Create calendar",
    calendarEditButton: "Edit",
    calendarEditDialogTitle: "Edit calendar",
    calendarDeleteButton: "Delete",
    calendarDeleteConfirm: "Delete this calendar and all its events?",
    calendarDeleteError: "Failed to delete calendar.",
    calendarColorLabel: "Calendar color",
    calendarGroupLabel: "Calendar group",
    calendarVisibilityError: "At least one calendar must remain visible.",
    weekGridNoCalendarsTitle: "No calendars",
    weekGridNoCalendarsHint: "Create a calendar in the sidebar to start planning.",
    weekOfPrefix: "Week of",
    daysShort: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"],
    eventFormCreateHeading: "New event",
    eventFormEditHeading: "Edit event",
    eventFormTemplateNoResults: "No templates found",
    eventFormTemplateAllDay: "All day",
    eventFormTemplateDays: "{count} days",
    eventFormTitle: "Title",
    eventFormCalendar: "Calendar",
    eventFormCalendarRequired: "Select a calendar.",
    eventFormNoCalendars: "Create a calendar first.",
    eventFormNoCalendarsPrompt: "You need at least one calendar before creating an event.",
    eventFormFocusCalendarCreate: "Go to calendar creation",
    eventFormStartDate: "Start date",
    eventFormEndDate: "End date",
    eventFormStartTime: "Start time",
    eventFormEndTime: "End time",
    eventFormAllDay: "All day",
    eventFormLocation: "Location",
    recurrenceRepeat: "Repeat",
    recurrenceNone: "Does not repeat",
    recurrenceWeekly: "Weekly",
    recurrenceMonthly: "Monthly",
    recurrenceEvery: "Every",
    recurrenceWeeks: "week(s)",
    recurrenceMonths: "month(s)",
    recurrenceOn: "On",
    recurrenceOnThe: "On the",
    recurrenceEnds: "Ends",
    recurrenceNever: "Never",
    recurrenceAfter: "After",
    recurrenceOccurrences: "occurrences",
    recurrenceOnDate: "On date",
    recurrenceOrdinal1: "1st",
    recurrenceOrdinal2: "2nd",
    recurrenceOrdinal3: "3rd",
    recurrenceOrdinal4: "4th",
    recurrenceOrdinal5: "5th",
    recurrenceEditPrompt: "Apply this change to all future events or just this one?",
    recurrenceEditAllFuture: "All future",
    recurrenceEditJustThisOne: "Just this one",
    bulkDescriptionPrompt: "{count} other events share this title. Update description on all?",
    bulkDescriptionUpdateAll: "Update all",
    bulkDescriptionOnlyThis: "Only this one",
    eventFormPrivateDescription: "Private description",
    eventFormPrivateDescriptionTooltip: "Private — excluded from public exports",
    eventFormPublicDescription: "Public description",
    eventFormDelete: "Delete",
    eventFormCancel: "Cancel",
    eventFormSave: "Save",
    eventFormDeleteConfirm: "Delete this event?",
    exportDialogTitle: "Export Calendar",
    exportModeLabel: "Export mode",
    exportModeFull: "Full export",
    exportModePublic: "Public export",
    exportModeFullSubtitle: "Both private and public events",
    exportModePublicSubtitle: "Public events only",
    exportEncryptLabel: "Encrypt export",
    exportEncryptHint: "Protect the file with a password",
    exportPasswordLabel: "Password",
    exportPasswordConfirmLabel: "Confirm password",
    exportCalendarsLabel: "Calendars",
    exportPreviewCount: "{count} events will be exported.",
    exportPreviewNone: "Select at least one calendar.",
    exportCalendarRequired: "Select at least one calendar.",
    exportPasswordRequired: "Enter a password and confirmation.",
    exportPasswordMismatch: "Passwords do not match.",
    exportCancel: "Cancel",
    exportConfirm: "Export",
    exportSuccessMessage: "Export complete: {path}",
    unlockTitle: "Ordning",
    unlockMessage: "Your data is encrypted. Enter password to unlock.",
    unlockPasswordLabel: "Password",
    unlockButton: "Unlock",
    unlockWrongPassword: "Wrong password.",
    unlockQuit: "Quit",
    importDialogTitle: "Import Calendar",
    importStrategyLabel: "Import strategy",
    importStrategyMerge: "Merge",
    importStrategyReplace: "Replace all",
    importPickFile: "Choose file",
    importNoFileSelected: "No file selected.",
    importPasswordLabel: "Password (for encrypted file)",
    importEncryptedFile: "Encrypted file",
    importUnlockButton: "Unlock",
    importUnlocked: "Unlocked",
    importPasswordHint: "Leave blank for unencrypted JSON.",
    importSummaryCalendars: "Calendars in file: {count}",
    importSummaryEvents: "Events in file: {count}",
    importSummaryNew: "New events: {count}",
    importSummaryUpdated: "Updated events: {count}",
    importSummaryConflicts: "Conflicts: {count}",
    importCancel: "Cancel",
    importConfirm: "Import",
    importSuccessMessage:
      "Import complete. New: {new}, updated: {updated}, conflicts: {conflicts}",
    offscreenMore: "More events",
    contextMenuOpen: "Open",
    contextMenuDelete: "Delete",
    contextMenuCopy: "Copy",
    contextMenuNewEvent: "New event",
    contextMenuPaste: "Paste",
    contextMenuCopyError: "Could not copy event.",
    purgePastButton: "Purge past events",
    purgePastPreview: "Delete all events in the calendar before today.",
    purgePastFinalConfirm: "This cannot be undone. Continue?",
    purgePastFinalCancel: "The past will persist",
    purgePastFinalProceed: "I will persist",
    purgePastSuccess: "{count} past events were deleted.",
    purgePastNone: "No past events to purge.",
    purgePastError: "Could not purge past events.",
    printWeekButton: "Print week",
    settingsButtonAria: "Open settings",
    settingsTitle: "Settings",
    settingsLanguageLabel: "Language",
    settingsTimezoneLabel: "Timezone",
    settingsLanguageSwedish: "Svenska",
    settingsLanguageEnglish: "English",
    settingsEncryptionTitle: "Encryption",
    settingsEncryptionOff: "Local data is not encrypted.",
    settingsEncryptionOn: "Local data is encrypted.",
    settingsEncryptButton: "Encrypt local data",
    settingsEncryptConfirmLabel: "Confirm password",
    settingsEncryptSubmit: "Encrypt",
    settingsEncryptCancel: "Cancel",
    settingsEncryptPasswordMismatch: "Passwords do not match.",
    settingsEncryptPasswordRequired: "Password required.",
    settingsEncryptedStatus: "On",
    settingsDisableEncrypt: "Disable encryption",
    multiDeleteConfirm: "Delete {count} events?",
    multiDeleteRecurringNote: "Recurring instances will be removed individually.",
    contextMenuDeleteCount: "Delete ({count})",
    closeUnsavedPrompt: "You have unsaved changes. What do you want to do?",
    closeUnsavedSave: "Save",
    closeUnsavedDontSave: "Don't save"
  }
};

const LOCALES = {
  sv: "sv-SE",
  en: "en-US"
};

let currentLang = "sv";

export function setLang(lang) {
  if (STRINGS[lang]) {
    currentLang = lang;
  }
}

export function getLang() {
  return currentLang;
}

export function getLocale() {
  return LOCALES[currentLang] ?? LOCALES.sv;
}

export function t(key) {
  const value = STRINGS[currentLang]?.[key];
  if (value !== undefined) {
    return value;
  }

  const fallback = STRINGS.sv?.[key];
  return fallback !== undefined ? fallback : key;
}

export function tDayShort(dayIndex) {
  const days = STRINGS[currentLang]?.daysShort ?? STRINGS.sv.daysShort;
  return days[dayIndex] ?? "";
}
